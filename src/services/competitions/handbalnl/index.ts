...
class HandbalNlLiveService implements HandbalNlService {
  ...
  public async retrieveData(tab: string, fromDate = '', toDate = '') {
    ...
    if ('programma' === tab) {
      formData.append(
        'filters',
        `{ "from": "${fromIsoDate(fromDate)}", "to": "${fromIsoDate(toDate)}", "home":[""] }`
      )
    }
    formData.append('excludedClubs', 'ZV452QZ ZV452DM ZV452JS ZT814KL')
    formData.append(
      'params[]',
      'NHV Landelijk | Zaal | Heren Super Handball League | Super Handball League'
    )
    formData.append('params[]', '37674')

    const { data, status } = await this.handbalNlApi.post('export-uitslagen', formData, {
      headers: {
...
export const handbalNlService = RETRIEVE_LIVE_DATA
  ? new HandbalNlLiveService()
  : new HandbalNlFileService()
