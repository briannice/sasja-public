...
class CompetitionService {
  ...
  public async getCompetitionCalendarFull(competition: TeamCompetition): Promise<GameDay[]> {
    const games =
      await getCompetitionIntegration(competition).getCompetitionCalendarFull(competition)
  ...
  public async getFutureGames(): Promise<GameDay[]> {
    ...
    const gamesByDate = games.flat().reduce(
      (acc, game) => {
        const key = game.date
        if (!acc[key]) {
          acc[key] = []
    ...
    return Object.entries(gamesByDate)
      ...
      .sort((day1, day2) => day1.date.localeCompare(day2.date))
  }
