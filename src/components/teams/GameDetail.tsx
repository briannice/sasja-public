...
export default function GameDetail({ game }: Props) {
    ...
    const createReferees = (refereeList: Referee[]) => {
        ...
        if (refereeList.length == 0) referees.push('Geen aanduidingen')
        else if (refereeList.length >= 1)
    ...
    const createAddress = (gameDetail: GameModel) => {
        ...
        if (gameDetail.venue_street) address.push(gameDetail.venue_street)
    ...
    const createMapAddress = (gameDetail: GameModel) => {
        ...
        if (gameDetail.venue_street) address += ', ' + gameDetail.venue_street
    ...
    return (
        <section key={gameDetail.date}>
            <div className="card divide-y divide-light overflow-auto">
                <div>
                    <p className="rounded-sm bg-primary px-1.5 py-0.5 font-kanit desktop:text-xl text-center text-white">
                        {gameDetail.serie_name}
                    </p>
                </div>
                <div key={gameDetail.id} className="p-4">
                    <div className="font-kanit text-center desktop:text-2xl text">
                        {createDate(gameDetail.date)}
                    </div>
                    <div className="mt-4 flex space-x-8">
                        <div className="flex flex-1 flex-col items-center justify-center">
                            <ClubLogo path={gameDetail.home_logo} size={width > 600 ? 120 : 40} />
                            <p className="text-center font-kanit desktop:text-2xl">
                                {gameDetail.home_name}
                            </p>
                        </div>
                        <div className="flex items-center justify-center space-x-4 font-kanit">
                            {gameDetail.game_status_id == 2 ? (
                                <p className="text-center desktop:text-3xl text-dark">
                    ...
                    <div>
                        ...
                        <div className="flex items-center justify-center space-x-1 text-sm">
                            {gameDetail.game_number}
                        </div>
                        {isAuthenticated() ? (
                            <div>
                                <div className="flex items-center justify-center space-x-1 text-sm">
                                    {gameDetail.match_code}
                                </div>
                                <div className="flex items-center justify-center space-x-1 text-sm">
                                    <AiOutlineHome />
                                    <span />
                                    {gameDetail.home_team_pin} | {gameDetail.away_team_pin}
                                    <AiOutlineCar />
                                    <span />
                                </div>
                            </div>
                    ...
                    <div>
                        ...
                        {!loading && (
                            <div className="flex items-center justify-center">
                                <Link
                                    className="m-2"
                                    href={`https://maps.apple.com/maps?q=${createMapAddress(gameDetail)}`}
                                    blank={true}
                                >
                                    <FaMapMarkedAlt />
                                </Link>
                                <Link
                                    className="m-2"
                                    href={`https://waze.com/ul?q=${createMapAddress(gameDetail)}`}
                                    blank={true}
                                >
                                    <FaWaze />
                                </Link>
