  ...
  return (
    <div className="bg-gradient-to-b from-transparent to-white">
      <div className="container flex flex-col items-center space-y-16 py-16">
        <div className="flex flex-wrap justify-center gap-8 tablet:gap-12 laptop:gap-16">
          {mainSponsors.map((sponsor) => (
            <a key={sponsor.name} href={sponsor.href} target="_blank" rel="noreferrer" className="">
              <figure className={cn('relative h-12 tablet:h-16 laptop:h-20', sponsor.aspect)}>
                <Image src={sponsor.path} alt="Sponsor image" layout="fill" objectFit="cover" />
              </figure>
            </a>
          ))}
        </div>
        <div className="mt-16 flex flex-wrap justify-center gap-8 tablet:gap-12 laptop:gap-16">
          {secondarySponsors.map((sponsor) => (
            <a key={sponsor.name} href={sponsor.href} target="_blank" rel="noreferrer" className="">
              <figure className={cn('relative h-8 tablet:h-10 laptop:h-12', sponsor.aspect)}>
                <Image src={sponsor.path} alt="Sponsor image" layout="fill" objectFit="cover" />
              </figure>
            </a>
          ))}
