...
export default function MatchReportOverview({ initialMatchReports, teamId = '' }: Props) {
    ...
    const loadMoreHandler: MouseEventHandler<HTMLButtonElement> = async (e) => {
        ...
        const lastDoc = await getDoc(
            doc(db, 'matchreport', matchReports[matchReports.length - 1].id)
        )
    ...
    return (
        <Container className="grid grid-cols-4 gap-8 tablet:grid-cols-8 laptop:grid-cols-12">
            <h2 className="title1 col-span-4 tablet:col-span-8 laptop:col-span-12">
                Matchverslagen
            </h2>
            {matchReports.map(
                (matchReport, i) =>
                    i < showing && (
                        <MatchreportCard key={matchReport.id} matchReport={matchReport} />
                    )
            )}
            {hasMore && (
